...
| `render debug_buffer <name>` | shows one of the buffers the frame is mixed from instead of the frame: `off`, `scene`, `light`, `bloom`, `emission`, `reflection`, `refraction`, `indirect`, `volumetric`, `dust`, `lens_flare`, `depth`, `position`, `normal`, `motion`. Written by `TextureMixerCS`, the last point where every contribution still exists separately. While one is selected, AA, motion blur, DOF and the lens effects are suppressed so you see the buffer and not a processed version of it — the stored settings are untouched and return when it goes back to `off` |
| `render debug_gain <float>` | exposure for the debug buffer views. The colour buffers are HDR: direct light saturates at 1x and indirect sits far below it, so `light` wants a gain under 1 and `indirect` wants 8 or more. Ignored for `depth`/`position`/`normal`, which are mapped rather than exposed |
