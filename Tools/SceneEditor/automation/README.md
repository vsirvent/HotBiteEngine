...
| `render debug_buffer <name>` | shows one of the buffers the frame is mixed from instead of the frame: `off`, `scene`, `light`, `bloom`, `emission`, `reflection`, `refraction`, `indirect`, `volumetric`, `dust`, `lens_flare`, `depth`, `position`, `normal`. Written by `TextureMixerCS`, the last point where every contribution still exists separately. While one is selected, AA, motion blur, DOF and the lens effects are suppressed so you see the buffer and not a processed version of it — the stored settings are untouched and return when it goes back to `off` |
